<template>
	<div>
		...
		子传子：<br>
		通过事件总线的方式传值（修改main.js） <br>
		<button @click="test=='abc'?test='two':test='abc'">切换</button>
		<component :is="test"></component> -->
	</div>
</template>

<script>
	import def from "./components/app4.vue"
	export default {
		data() {
			return {
				test: "abc",
		...
		components: {
			abc: def,
			two: () => import("./components/app-4.vue")
		},
		methods:{
			load(val){
				console.log(val)
